...
public class BTreeFile extends IndexFile implements GlobalConst {
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			if (upEntry != null) {
				...
				if (currentIndexPage.available_space() >= entrySize) {
					...
					// 1) Collect ALL entries from old page + the new upEntry
					java.util.ArrayList<KeyDataEntry> allEntries = new java.util.ArrayList<>();
					RID scanRid = new RID();
					for (KeyDataEntry e = currentIndexPage.getFirst(scanRid); e != null; e = currentIndexPage
							.getNext(scanRid)) {
						allEntries.add(e);
					}
					// Delete them from currentIndexPage
					for (KeyDataEntry e : allEntries) {
						currentIndexPage.deleteSortedRecord(new RID(scanRid.pageNo, scanRid.slotNo));
					}
					allEntries.clear(); // we'll re-scan below

					// Re-scan the original index page
					scanRid = new RID();
					KeyDataEntry temp;
					while ((temp = currentIndexPage.getFirst(scanRid)) != null) {
						currentIndexPage.deleteSortedRecord(scanRid);
					}

					// Now put them in an array again
					allEntries = new java.util.ArrayList<>();
					// Insert them from scratch (including the new upEntry)
					// Instead of "reinserting," simpler is to gather everything in memory, sort,
					// then redistribute
					allEntries.add(upEntry); // the new child pointer

					// Also gather old entries
					RID ridIter = new RID();
					for (KeyDataEntry e = currentIndexPage.getFirst(ridIter); e != null; e = currentIndexPage
							.getNext(ridIter)) {
						allEntries.add(e);
					}

					// Actually, we need them from the original list. Let's do a simpler approach:
					// Re-pin the original index page again, gather from scratch:
					unpinPage(currentPageId, false);
					pinPage(currentPageId);
					currentIndexPage = new BTIndexPage(page, headerPage.get_keyType());

					// Gather everything again (since we might have confusion).
					...
					// Actually, in a B+ tree, the 'midEntry' stays out of the pages. It's promoted
					// up.
					// So let's make sure the midEntry isn't physically in either page if we
					// inserted it:
					RID delRid = new RID();
			...
			if (currentLeafPage.available_space() >= entrySize) {
				...
				java.util.ArrayList<KeyDataEntry> allEntries = new java.util.ArrayList<>();
				RID scanRid = new RID();
				for (KeyDataEntry e = currentLeafPage.getFirst(scanRid); e != null; e = currentLeafPage
						.getNext(scanRid)) {
					allEntries.add(e);
				}
	...
	public boolean NaiveDelete(KeyClass key, RID rid)
		...
		// Step 1: Find the starting leaf page
		RID startrid = new RID();
		BTLeafPage leafPage = findRunStart(key, startrid);

		if (leafPage == null) {
			return false; // Key not found
		}

		// Step 2: Get the current entry
		KeyDataEntry entry = leafPage.getCurrent(startrid);

		// Step 3: Iterate through the entries
		while (true) {
			// Step 3.1: Handle null entries (go to the next page)
			while (entry == null) {
				PageId nextPage = leafPage.getNextPage();
				unpinPage(leafPage.getCurPage(), false); // Unpin the current page
				...
				entry = leafPage.getFirst(startrid);
			}

			// Step 3.2: Compare keys
			if (BT.keyCompare(entry.key, key) == 0) {
				// Step 3.3: Ensure entry.data is a LeafData
				if (entry.data instanceof LeafData) {
					RID entryRid = ((LeafData) entry.data).getData();
					if (entryRid.equals(rid)) {
						// Step 3.4: Delete the entry if the key and RID match
						if (leafPage.deleteSortedRecord(startrid)) {
							unpinPage(leafPage.getCurPage(), true); // Mark as dirty
			...
			// Step 3.5: Move to the next entry
			entry = leafPage.getNext(startrid);
		}
	}
	...
	public BTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
		...
		// sets up scan at the starting position
